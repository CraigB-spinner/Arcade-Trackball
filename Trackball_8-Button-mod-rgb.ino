...
//#define NORM true//Mouse movement Normal; every interrupt processed; maybe lag in fast movement, 2 pulse = 2 move
//#define DROP true//Mouse movement Drop; drop extra interrupts processed; smooth movement, no lag, 2 - 2n+1 pulse = 1 move
...
#define rgbTrack //turn on RGB Trackball colour rotation
...
#define xRGBgrn 0b00000010 //Green colour
...
// *** Trackball movement logic ***
  ...
  //(NORM)-Accelerated move or (DROP)-Smooth drop extra moves.
  if(rotPositionX >= 2 || rotPositionX <= -2) {
    rotMoveX = (rotPositionX > 0) ? 1 : -1;
    Mouse.move(rotMoveX,0,0);
  #ifdef NORM
    rotPositionX -= (rotMoveX << 1);
  #else 
    rotPositionX = 0;
  #endif 
    moveMouse = true;
  ...
  if(rotPositionY >= 2 || rotPositionY <= -2) {
    rotMoveY = (rotPositionY > 0) ? 1 : -1;
    Mouse.move(0,rotMoveY,0);
  #ifdef NORM
    rotPositionY -= (rotMoveY << 1);
  #else
    rotPositionY = 0;
  #endif 
    moveMouse = true;
